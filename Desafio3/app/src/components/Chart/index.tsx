...
const Chart: React.FC<ChartProps> = (props) => {
    ...
    const cadleSeriesRef = React.useRef() as React.MutableRefObject<ISeriesApi<"Candlestick">>
    ...
    React.useEffect(() => {
        const interval = setInterval(() => {
            ...
            cryptoHttp.get(`histominute?fsym=${coin}&tsym=BRL&limit=1`).then((response) => {
                setPrices((prevState) => {
                    ...
                    cadleSeriesRef.current.update(newPrice)
    ...
    React.useEffect(() => {
        if (cadleSeriesRef.current) {
            cadleSeriesRef.current.setData(prices)
        }
    }, [prices])
    ...
    React.useEffect(() => {
        ...
        cadleSeriesRef.current = chart.addCandlestickSeries({
            upColor: "#4bffb5",
            downColor: "#ff4976",
